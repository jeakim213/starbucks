							...
							<fieldset>
								...
								<section class="renew_joinform_v2">
									<div class="renew_input_box gender_chk">
										...
										<p class="nofix_name">김준희</p>
										<input type="hidden" id="user_nm" name="user_nm" value='김준희' required="required" /><!-- 접근성_20171120 required 추가 -->
							...
							<!-- //20180726 영역삭제 -->
								...
								<img src="https://image.istarbucks.co.kr/common/img/util/mem/icon_nick_sally.jpg" alt="닉네임 별부자 고객님, 주문하신 바닐라 라떼 나왔습니다!" /> <!-- 접근성_20171120   alt  문구 추가 -->
									...
									<div class="renew_input_box user_nick_nm_chk bd_none">
										...
										<!-- //20180726 이용약관 영역추가 -->
										<label for="user_nick_nm" class="hid">닉네임</label> <input type="text" name="user_nick_nm" id="user_nick_nm" placeholder="닉네임 입력을 위해 약관에 동의해 주세요." maxlength="6" disabled=""/><!-- 20180726 수정 -->
			...
			,success:function(response){
				...
				if (jsonStr != null) {
					...
					if (errCode == "0"
						...
						$("#tbody_adr_result").html('<tr><td colspan="3">검색결과가 없습니다.</td></tr>');
					}
				} else {
					$("#tbody_adr_result").html('<tr><td colspan="3">검색결과가 없습니다.</td></tr>');
				}
		...
		<script>
			...
			function initClause() {
				var objParam = {
			 	...
			 	__ajaxCall("/interface/reqWebClauseData.do", objParam, true, "json", "post"
						, function (_response) {
						if (_response.resultCode == "0000") {
						    $("#nickname_text").html(_response.data.contents);
						}else{
			...
			function setEvents() {
				...
				$("#user_pwd").on("blur", function() {
					...
					if (isValid) {
						...
						__ajaxCall("/interface/getUserPwdCheck.do", objParam, true, "json", "post"
							, function (_response) {
								if (_response.result_code == "SUCCESS") {
				...
				$("#mem_choice_yes").on("change", function() {
					...
					if ($chk.length == 0) {
						...
						$("#user_nick_nm").attr("disabled", true);
				...
				$("#user_nick_nm").on("blur change", function() {
					...
					if (user_nick_nm == "") {
						...
						if (isValidNickname(user_nick_nm) == false) {
							...
							___ajaxCall("/interface/checkNicknameValid.do", objParam, true, "json", "post"
								, function (_response) {
									if (_response.result_code != "SUCCESS") {
										$("#user_nick_nm").addClass("input_warn");
			...
			function checkUserId() {
				...
				__ajaxCall("/interface/getUserIdCheck.do", objParam, true, "json", "post"
					, function (_response) {
			...
			function joinMember() {
				...
				if (confirm("입력하신 정보를 다시 한번 확인해주세요.\n\n＊휴대폰 번호 : "+ phone +"\n＊이메일 : "+ email)) {
					...
					objParam.clauseYns = "Y,Y," + $('[name="nick_nm_use"]').val();
				 	...
				 	__ajaxCall("/interface/joinMember.do", objParam, true, "json", "post"
						, function (_response) {
							if (_response.alert_msg != "") {
							...
							if (_response.result_code == "SUCCESS") {
								if (objParam.join_flag == "3") {	// 휴대폰 인증 인가?
									...
									___ajaxCall("/interface/setPhoneCertLogForJoin.do", objPhoneCertParam, false, "json", "post"
										,function (_response) {
										}
									);
								}
			...
			function initNickLimited() {
				__ajaxCall("/interface/checkNicknameLimitedYn.do", '', true, "json", "post"
=======
<link href="../common/css/reset.css" rel="stylesheet">
<link href="../common/css/style.css?v=210721" rel="stylesheet">
<link href="../common/css/jquery.bxslider.css" rel="stylesheet">
