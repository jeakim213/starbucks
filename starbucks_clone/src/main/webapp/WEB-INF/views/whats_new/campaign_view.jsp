...
<input type="hidden" id="listPageName" value="/whats_new/campaign_list">
<input type="hidden" id="viewPageName" value="/whats_new/campaign_view">
...
<script>
	...
	var $VIEW_YN = "";
          ...
          function getIngList() {
              ...
              __ajaxCall('/whats_new/getIngMax4List', option , true, "JSON", "POST", 
              function(data) {
          ...
          function getDataInfo() {
              ...
              __ajaxCall('/whats_new/productViewAjax', option , true, "JSON", "POST", 
              function(data) {
              	if(data.view.length > 0) {
                      var data = data.view[0];
                      $('.campaign_veiw_wrap > header > h3')(data.title + '<span class="date"></span>');
					...
					if($PRO_SEQ == "1085"){
                      ...
                      if($("#eventPageName").val() != "RESERVE_MAGAZINE") { //리저브 페이지일 경우 기간 미노출
                    	  $('.campaign_veiw_wrap > header > h3 > span')(dateStr);  
                      }
                      
                      if(data.store_GUBUN == 'A') {
                      	$('p.s_t')('<span>전매장</span> 에서 확인하실 수 있습니다.');
                      } else if(data.store_GUBUN == 'B') {
                      	if(data.store_CNT == 0) {
                              $('p.s_t')('');
                      	} else if(data.store_CNT == 1) {
                              $('p.s_t')('<span>'+data.store_NM+'</span> 에서 확인하실 수 있습니다.');
                      	} else {
                              $('p.s_t')(data.store_NM + ' 외 <span>'+(data.store_CNT - 1)+'개 매장</span> 에서 확인하실 수 있습니다.');
                      	}
                      }
                      ...
                      if($PRO_SEQ != "1149" && $PRO_SEQ != "1131" && $PRO_SEQ != "1036" && $PRO_SEQ != "1040" && $PRO_SEQ != "1047" && $PRO_SEQ != "1048" && $PRO_SEQ != "1053"
                    	  ...
                    	  if (data.contn_CNTNT_XPSR_DVSN_CODE == 'STB3501') {
                    			...
                    			$('.campaign_veiw_info_inner')(content);
                    		} else {
                    			$('.campaign_veiw_info_inner')(data.content || '');
                    		}
